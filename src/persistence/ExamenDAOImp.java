...
public class ExamenDAOImp implements ExamenDAO {
	...
	public List<Actividad> obtenerTodasActividades() {
		...
		try{
			connectionManager.connect();
			ResultSet listExamenesResultSet = connectionManager.queryDB("SELECT * from ACTIVIDAD A, EXAMEN E WHERE A.id_actividad = E.id_actividad AND A.para_despues = FALSE AND A.finalizada = FALSE ");
			connectionManager.close();
	...
	public List<Actividad> obtenerActividadesHoy() {
		...
		try{
			connectionManager.connect();
			ResultSet listExamenesResultSet = connectionManager.queryDB("SELECT * from ACTIVIDAD A, EXAMEN E WHERE A.id_actividad = E.id_actividad AND A.fecha_finalizacion = '" + Date_solver.convertirLocalDateEnSQL(Date_solver.fechaDeHoy())+ "' AND A.para_despues = FALSE AND A.finalizada = FALSE ");
			connectionManager.close();
